...
def novo_caso(request):
    if request.method == 'POST':
        form = CasoClinicoForm(request.POST, request.FILES)
        if form.is_valid():
...
def editar_caso(request, pk):
    ...
    if request.user != caso.autor:
        messages.warning(request, 'Você não tem permissão para editar este caso.')
        return redirect('detalhe_caso', pk=pk)
    if request.method == 'POST':
        form = CasoClinicoForm(request.POST, request.FILES, instance=caso)
        if form.is_valid():
            form.save()
